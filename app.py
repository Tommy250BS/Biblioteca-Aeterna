...
# Le domande dell'Ephemeris vivono in un file JSON a parte
# (ephemeris_domande.json, nella stessa cartella di questo file) invece che
# come literal Python qui dentro: aggiungerne di nuove, o scorrere quelle
# già scritte per non ripeterle, non richiede più aprire app.py e cercare
# tra centinaia di righe di codice — è testo puro, organizzato per tipo.
# _carica_ephemeris_banco() lo appiattisce in un'unica lista di dict con
# la chiave "tipo" reintegrata in ciascuno: stessa identica forma della
# vecchia EPHEMERIS_BANCO letterale, quindi tutto il resto del file
# (rotazione dei tipi, validazione delle risposte, ecc.) non cambia.
def _carica_ephemeris_banco():
    path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ephemeris_domande.json")
    with open(path, encoding="utf-8") as f:
        per_tipo = json.load(f)
    banco = []
    for tipo, domande in per_tipo.items():
        for d in domande:
            banco.append({"tipo": tipo, **d})
    return banco

EPHEMERIS_BANCO = _carica_ephemeris_banco()
